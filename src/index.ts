...
export default {
	async fetch(request: Request, env: Env, context: ExecutionContext) {
		...
		try {
			...
			//
			// TODO(jqphu): We need to wait for the prod request to complete. If we don't await here and try to return the response. The worker fails as it tries to read the body after the response has already been sent.
			...
			//
			// TODO(jqphu): cloning the response here is going to be paid for every request. Probably need to benchmark this if we are sending a lot of data.
